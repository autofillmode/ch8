...
main(int argc, char *argv[])
{
	...
	SDL_CreateWindowAndRenderer(WINDOW_W, WINDOW_H, SDL_WINDOW_SHOWN,
				    &gWindow, &renderer);
...
drawScreen(SDL_Renderer *renderer, uint8_t screen[64][32])
{
	for (int x = 0; x < 64; x++) {
		for (int y = 0; y < 32; y++) {
			SDL_SetRenderDrawColor(renderer, screen[x][y] ? 255 : 0,
					       screen[x][y] ? 255 : 0,
					       screen[x][y] ? 255 : 0,
					       screen[x][y] ? 255 : 0);
	...
	case 0xD: {
		...
		for (unsigned int row = 0; row < n; ++row) {
			...
			for (unsigned char col = 0; col < 8; ++col) {
				...
				uint8_t *screenpxl =
					&screen[p_x + col][p_y + row];
				registers[0xF] =
					*screenpxl; /* 1 if on, 0 if off */
				*screenpxl ^= spritepxl;
			}
		}
		draw_flag = 1;
